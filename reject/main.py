...
def calculate_bits_per_word(binary_message, covertext, context, tokenizer):
    ...
    print(f"\n{total_bits}Bits per {encoded_tokens}Word: {bits_per_word:.4f}")
...
def test_single(**kwargs):
    ...
    # with open("outputencode.txt", "w", encoding="utf-8") as f:
    #     f.write(covertext)
    ...
    # print("\nDecoding message...")
    # evaluator.clear()
    # print(evaluator.threshold,evaluator.all_log_list)
    ...
    return bits_per_word, ppl, kl, covertext
...
def load_messages_from_txt(txt_file):
    ...
    with open(txt_file, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if message:  # 确保不是空行
                messages.append(message[:500])  # 截断超长文本
    return messages
...
def main():
    ...
    for dataset_name in ["druge","newse","covide"]:#,"covid450","drugr"   "news" "news", "drug", "covid"
        # if dataset_name == "druge":
        #     continue
        ...
        # 指定 CSV 文件路径
        csv_file = f"./result/{dataset_name}human_8_102050_reject.csv"  
        # 先写入 CSV 头部
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["message", "length", "bpw", "ppl", "kl", "covertext"])  # 写入表头
        ...
        for m in messages:
            ...
            bpw, ppl, kl, covertxt = test_single(**params)
            bpw_info.append(bpw)
            ppl_info.append(ppl)
            kl_info.append(kl)
            ...
            with open(csv_file, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([m, len(m), bpw, ppl, kl, covertxt])

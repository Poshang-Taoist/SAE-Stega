...
def encode_arithmetic(model, enc, message, context, finish_sent=False, device='cuda', temp=1.0, precision=16,
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            if i >= len(message):
                ...
                logq = q.log()
                cur_step_kl = kl(q, logq, log_probs[:len(q)])
                kl_list.append(cur_step_kl)
                total_kl += cur_step_kl
    ...
    return output[len(context):].tolist(), avg_NLL, avg_KL, words_per_bit, avg_Hq, kl_list

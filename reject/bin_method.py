...
# |Ġonce||Ġbanned||:||Ġantis||em||itism||.||ĠWe||Ġhear||Ġso||Ġwell||Ġabout||Ġthe||Ġrise|
    ...
    def encode_message(self, message, context, evaluator=None):
        ...
        with torch.no_grad():
            while concealed_length < len(message):
                if len(output) > 256:
                    cut_output.append(output[0].item())
                    output = output[1:]
                if self.embedding_method == 'static':
                    ...
                    concealed_length, output, success, kl = self._encode_rejection_bin(message, concealed_length, output, evaluator)
                # if not success:
                #     # 获取当前 token 的下标位置
                ...
                tot_kl += kl
                cnt += 1
        # output = torch.cat((cut_output, output))
        output = torch.cat((torch.tensor(cut_output, device=self.device, dtype=torch.long), output))
        ...
        return covertext, output.tolist(), tot_kl/cnt
    ...
    def _encode_rejection_bin(self, message, concealed_length, output, evaluator):
        ...
        if success:
            ...
            log_probs = F.log_softmax(logits, dim=-1) #LM Distribution
            probs_stego = F.softmax(filtered_logits, dim=-1) #Stego Distribution
            logs = probs_stego.log()
            # kl = F.kl_div(log_probs, probs_stego, reduction='sum')
            kl = kl1(probs_stego,logs,log_probs[:len(probs_stego)])#
        else:
        ...
        return concealed_length, output, success, kl
    ...
    def _decode_rejection_bin(self, token, output, decoded_message, evaluator, reject):
        ...
        if success:
            ...
            # Convert the bin index to binary and append to the decoded message
            evaluator.add_success_count()
            binary_str = bin(bin_num)[2:].zfill(block_size)
            decoded_message.extend([int(bit) for bit in binary_str])
        else:
            # 如果评估失败，则跳过该token
            # Skip this token if it fails evaluation
            evaluator.add_failure_count()
            evaluator.adjust_threshold()
            # token = torch.argmax(logits).item()

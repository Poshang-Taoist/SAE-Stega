...
def get_model(seed=1234, model_name='gpt2', device_id="0"):
    ...
    enc = GPT2Tokenizer.from_pretrained(r"D:\UIR\信息隐藏-从数据的角度研究\StegaText-master-original-version\gpt2m")
    enc.unk_token = None
    enc.bos_token = None
    enc.eos_token = None
...
def get_output_file_name(args):
    ...
    if steganography_method in ["bins", "huffman"]:
        ...
        output_name = f"{dataset_path}/Kresults_{encryption_method}_{steganography_method}_block_{block_size}.csv"
    elif steganography_method == "patient-huffman":
        block_size = args["block_size"]
        epsilon = args["epsilon"]
        output_name = f"{dataset_path}/Kresults_{encryption_method}_{steganography_method}_block_{block_size}_epsilon_{epsilon}.csv"
    elif steganography_method == "arithmetic":
        precision = args["precision"]
        temp = args["temp"]
        topK = args["topK"]
        output_name = f"{dataset_path}/Kresults_{encryption_method}_{steganography_method}_precision_{precision}_temp_{temp}_topK_{topK}.csv"
    elif steganography_method == "saac":
        precision = args["precision"]
        temp = args["temp"]
        delta = args["delta"]
        output_name = f"{dataset_path}/Kresults_{encryption_method}_{steganography_method}_precision_{precision}_temp_{temp}_delta_{delta}.csv"
    else:
        output_name = ""
    return output_name

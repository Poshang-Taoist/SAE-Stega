...
def main(args):
    ...
    if dataset != "random":
        with open(f"{dataset_path}/plaintext.txt", "r", encoding="utf8") as fin:
            plaintexts = [line.strip()[:500] for line in fin.readlines() if line.strip() != ""]
    ...
    encoding_infos = []
    encoding_context = "By now, many Americans are"
    covertexts = [] 
    print(f"Steganography Encoding Algorithm: {steganography_method}")
    output_name = get_output_file_name(args)
    with open(output_name, "w", newline="", encoding='utf-8') as fout: 
        writer = csv.writer(fout)
        writer.writerow(["id","covertext", "bpw", "ppl", "kl", "bit"])  # 写入表头
    cnt = -1
    start = time.time()
    for message in tqdm(messages, desc="encoding"):
        cnt += 1
        # if cnt < 3850:
        #     continue
        ...
        kl = info.get("kl", None)  # 获取 KL
        bpw = info.get("bits_per_word", None)  # 获取 BPW
        ppl = info.get("ppl", None)  # 获取 PPL
        with open(output_name, "a", newline="", encoding='utf-8') as fout: 
            writer = csv.writer(fout)
            writer.writerow([cnt, encoding_context+covertext, bpw, ppl, kl, len(message)])
    end = time.time()
    efficiency = (end-start) / len(messages)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-dataset", type=str, default="drug", choices=["druge","newse","covide","drug", "cnn_dm", "covid_19", "random"])
    parser.add_argument("-dataset_path", type=str, default="./datasets/drug/")
    parser.add_argument("-encrypt", type=str, default="utf8", choices=["utf8", "arithmetic", "cached"])
    ...
    parser.add_argument("-device", type=str, default="cpu", help="your gpu device id")
